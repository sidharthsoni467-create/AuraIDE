...
function createEditor(container, additionalExtensions = []) {
  const cursorListener = EditorView.updateListener.of((update) => {
    if (update.selectionSet) {
      const state2 = update.state;
      const head = state2.selection.main.head;
...
// client/src/collab.js
var ws = null;
var localRevision = 0;
var pendingOps = [];
var username = "";
function setupCollaboration(roomName, name2, userColor) {
  username = name2;
  ws = new WebSocket(`ws://localhost:9090`);
  ws.onopen = () => {
    console.log("[Collab] Connected to C++ server");
    sendMessage({ type: "join", client_id: name2, room: roomName, color: userColor });
  };
  ws.onmessage = (event) => {
...
function sendOp(type, position, text, length = 0) {
  const op = {
    type,
    position,
    text,
...
// client/src/autocomplete.js
var setGhostText = StateEffect.define();
var ghostTextState = StateField.define({
  create() {
    return null;
  },
  update(value, tr) {
...
var ghostTextDecorations = ViewPlugin.fromClass(class {
  constructor(view) {
    this.decorations = this.getDeco(view);
  }
  update(update) {
...
var aiAutocompleteListener = ViewPlugin.fromClass(class {
  update(update) {
    if (update.docChanged || update.selectionSet) {
      clearTimeout(fetchTimeout);
      fetchTimeout = setTimeout(async () => {
        const head = update.view.state.selection.main.head;
        const from = Math.max(0, head - 1e3);
...
var acceptGhostTextCommand = (view) => {
  const text = view.state.field(ghostTextState);
  if (text) {
    const pos = view.state.selection.main.head;
    view.dispatch({
  ...
  const updateUsers = () => {
    ...
    states.forEach((state) => {
      if (state.user) {
        ...
        dot2.className = "dot";
        dot2.style.backgroundColor = state.user.color;
        const name2 = document.createTextNode(state.user.name);
        tag.appendChild(dot2);
        tag.appendChild(name2);
...
function updateConnectionStatus(connected) {
  const dot2 = document.getElementById("connection-status");
  if (connected) {
    dot2.classList.remove("disconnected");
    dot2.classList.add("connected");
  } else {
    dot2.classList.remove("connected");
    dot2.classList.add("disconnected");
  }
}

// client/src/app.js
var remoteTransaction = Annotation.define();
document.addEventListener("DOMContentLoaded", () => {
  ...
  form.addEventListener("submit", (e) => {
    const username2 = input.value.trim() || "Anonymous";
    localStorage.setItem("aura-username", username2);
    ...
    for (let i = 0; i < username2.length; i++) {
      hash2 = username2.charCodeAt(i) + ((hash2 << 5) - hash2);
    }
    const colorIndex = Math.abs(hash2) % userColors.length;
    const userColor = userColors[colorIndex];
    const { ws: ws2 } = setupCollaboration(roomName, username2, userColor);
    const collabListener = EditorView.updateListener.of((update) => {
      if (update.docChanged && !update.transactions.some((t2) => t2.annotation(remoteTransaction))) {
    ...
    window.addEventListener("remote-op", (e2) => {
      const op = e2.detail;
      if (op.type === "insert") {
        view.dispatch({
    ...
    setupAwareness({});
    ws2.onopen = () => updateConnectionStatus(true);
    ws2.onclose = () => updateConnectionStatus(false);
  });
});
